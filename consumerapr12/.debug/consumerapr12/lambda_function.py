...
def create_language(row):
    """Uses AWS Comprehend to Create Sentiments on a DataFrame"""

    LOG.info(f"Processing {row}")
    comprehend = boto3.client(service_name='comprehend')
    payload = comprehend.detect_dominant_language(Text = row)
    LOG.debug(f"Language detected: {payload}")
    language = payload['Languages'][0]['LanguageCode']
    return language

def apply_language(df, column="wikipedia_snippit"):
    """Uses Pandas Apply to Create language Analysis"""

    df['Language'] = df[column].apply(create_language)
    return df
...
def create_entity(row):
    ...
    LOG.debug(f"entity detected: {payload}")
    entities = payload['Entities']
    text_lst = []
    for i in range(len(entities)):
        text_lst.append(entities[i]['Text'])
...
def apply_entities(df, column="wikipedia_snippit"):
    ...
    df['entity'] = df[column].apply(create_entity)
    return df
